...
MongoClient.connect('mongodb://localhost:27017', function(err, db) {
    ...
    var done = 0;
    var mydb = db.db('social');

    screenNames.forEach(function(name) {

        var cursor = mydb.collection("statuses").find({"user.screen_name": name});
        cursor.sort({ "id": -1 });
        cursor.limit(1);
        ...
        cursor.toArray(function(err, docs) {
            ...
            var params;
            if (docs.length == 1) {
                params = { "screen_name": name, "since_id": docs[0].id, "count": 200 };
            } else {
            ...
            client.get('statuses/user_timeline', params, function(err, statuses, response) {
                ...
                mydb.collection("statuses").insertMany(statuses, function(err, res) {
